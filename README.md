...
### 📌 Варианты сборки
- **Два типа сборки:**
  <br>`[1G1R] 1 РОМ - 1 ИГРА` - выбирает только одну версию игры (более позднюю и с учётом ПРИОРИТЕТА РЕГИОНОВ)
  <br>`[FULL] ВСЕ ВЕРСИИ` - создаёт папку для каждой игры со всеми региональные версиями и ревизиями
